...
@section('title', __('Pending interests payouts'))

@section('trans', __('active'))
@section('trans-pend-int', __('active'))
					...
					<table class="table table-striped table-hover" id="tableT">
						...
						<tbody>
							...
							@foreach($payouts as $post)
								<tr>
									<td>{{ $i++ }}</td>
									<td>{!! "<img class='profile-img-circle box-center' style='width:120px;height:120px;object-fit:cover;' src='".asset($post->investment->user->profile_photo_path)."' alt='".$post->investment->user->firstname."'>" !!}</td>
									<td>{{ '₦'.number_format($post->amount,2) }}</td>
									<td>{{ $post->status }}</td>
									<td>{{ date('F d, Y h:i a', strtotime($post->created_at)) }}</td>
									<td>{!! "<a href='/admin/investments/view/".$post->investment->id."' class='btn btn-outline-info btn-sm mr-2' target='_blank'>View Investment</a><a data-target='".url('/admin/investments/interests/pay/'.$post->id)."' class='btn btn-outline-success btn-sm performing'>Pay</a>" !!}
								</tr>
							@endforeach
						</tbody>
					</table>
